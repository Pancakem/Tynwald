...
{
  return parse_exp_new(input);
}
void parse_destroy(parse_cxt_t *cxt) { parse_exp_destroy(cxt); }

/**
 * @brief Top-level parsing, i.e., global level parsing
    ...
    while (1)
    {
      ...
      { // case 3
        ...
        if (token_lookahead_notnull(cxt->token_cxt, 1)->type == T_LCPAREN)
          ...
          ast_append_child(var, parse_exp(cxt, PARSE_EXP_NOCOMMA));
        la = token_lookahead_notnull(cxt->token_cxt, 1);
      }
      if (la->type == T_SEMICOLON)
      { // case 4
        token_consume_type(cxt->token_cxt, T_SEMICOLON);
        break;
      }
