...
parse_decl_cxt_t *parse_comp_new(char *input) { return parse_exp_new(input); }
void parse_comp_destroy(parse_comp_cxt_t *cxt) { parse_exp_destroy(cxt); }

/**
 * @brief This parses struct or union or enum
 * 
 * @param cxt 
 * @return token_t* 
 */
token_t *parse_comp(parse_exp_cxt_t *cxt)
{
  ...
  case T_STRUCT:
  case T_UNION:
    return parse_struct_union(cxt, token);
  case T_ENUM:
    return parse_enum(cxt, token);
  default:
    assert(0);
  }
}
...
/**
 * @brief Returns 1 if there is a body, 0 if no body; Name is pushed into root as either
...
/**
 * @brief Returns the same node which is either T_STRUCT or T_UNION
...
token_t *parse_struct_union(parse_comp_cxt_t *cxt, token_t *root)
{
    ...
    { // loop on lines
      ...
      { // loop on fields
        ...
        else if (la->type == T_SEMICOLON)
        {
          ...
          break;
        } // Finish parsing the field on ';'
        else
        {
          error_row_col_exit(la->offset, "Unexpected symbol \"%s\" in struct/union field declaration\n",
  ...
  {
    ast_append_child(root, token_get_empty());
  } // Otherwise append an empty child to indicate there is no body
  return root;
}
...
token_t *parse_enum(parse_comp_cxt_t *cxt, token_t *root)
{
  if (parse_name_body(cxt, root))
  {
    ...
    { // loop on lines
      ...
      token_t *la = token_lookahead_notnull(cxt->token_cxt, 1);
      if (la->type == T_IDENT)
        ast_append_child(enum_field, token_get_next(cxt->token_cxt));
      else
        error_row_col_exit(la->offset, "Expecting an identifier in enum body\n");
      la = token_lookahead_notnull(cxt->token_cxt, 1);
      if (la->type == T_ASSIGN)
      {
      ...
      {
        error_row_col_exit(la->offset, "Unexpected symbol \"%s\" in enum body\n",
                           token_typestr(la->type));
      }
    }
  }
