...
void token_cxt_destroy(token_cxt_t *cxt) {
  while(stack_size(cxt->udef_types) != 0) {
    ht_destroy(stack_pop(cxt->udef_types));
  }
  stack_destroy(cxt->udef_types);
  token_t *curr = cxt->pb_head;
  while(curr != NULL) {
    token_t *next = curr->next;
    token_destroy(curr);
...
/**
 * @brief Called by parse_stmt when we see a statement block
 * 
 * @param cxt 
 */
...
void token_exit_scope(token_cxt_t *cxt) { 
  ht_destroy((hashtable_t *)stack_pop(cxt->udef_types));
}

/**
 * @brief Adds a user-defined type into current scope's hash table. The parser adds a name when
...
void token_add_utype(token_cxt_t *cxt, token_t *token) {
  ...
  if(prev != HT_NOTFOUND) {
    int row_, col_;
    error_get_row_col(cxt->begin, &row_, &col_);
    error_row_col_exit(token->offset, 
      "The type name \"%s\" for typedef has already been defined @ row %d col %d\n", token->str, row_, col_);
  }
  ht_insert((hashtable_t *)stack_peek(cxt->udef_types), token->str, token);
}

/**
 * @brief Search from stack top to stack bottom and see whether we defined that type
 * 
 * @param cxt 
 * @param token 
 * @return int 
...
/**
 * @brief Apply the token's specifier/qualifier/type to the prop and return updated value
...
/**
 * @brief Returns a buffer that contains string representation of the property bit mask
 * 
 * @param decl_prop 
 * @return char* 
 */
...
/**
 * @brief Return T_ILLEGAL if not a keyword; keyword type otherwise
...
/**
 * @brief Return the actual symbol of the raw token type; do not work for some AST nodes
