...
#define error_exit(fmt, ...)                           \
    ...
    {                                                  \
        ...
        error_exit_or_jump(ERROR_ACTION_EXIT);         \
    } while (0);
#define error_row_col_exit(s, fmt, ...)                                          \
    do                                                                           \
    {                                                                            \
        int row, col;                                                            \
        error_get_row_col(s, &row, &col);                                        \
        fprintf(stderr, "Error (row %d col %d): " fmt, row, col, ##__VA_ARGS__); \
        error_exit_or_jump(ERROR_ACTION_EXIT);                                   \
    } while (0);
#define warn_row_col_exit(s, fmt, ...)                                             \
    do                                                                             \
    {                                                                              \
        int row, col;                                                              \
        error_get_row_col(s, &row, &col);                                          \
        fprintf(stderr, "Warning (row %d col %d): " fmt, row, col, ##__VA_ARGS__); \
        error_exit_or_jump(ERROR_ACTION_CONT);                                     \
    } while (0);
...
#define SYSEXPECT(expr)         \
    ...
    {                           \
        if (!(expr))            \
            syserror(__func__); \
    } while (0) // Assertion for system calls; Valid under all modes

void error_new(const char *s);
