...
parse_decl_cxt_t *parse_decl_new(char *input) { return parse_exp_new(input); }
void parse_decl_destroy(parse_decl_cxt_t *cxt) { parse_exp_destroy(cxt); }

/**
 * @brief Whether the token could start a declaration, i.e. being a type, modifier, or udef type
 * 
 * @param cxt 
 * @param token 
 * @return int 
...
{
  (void)cxt;
  return ((token->decl_prop & DECL_MASK) || token->type == T_UDEF) ? 1 : 0;
}

/**
 * @brief Same rule as parse_exp_next_token()
...
token_t *parse_decl_next_token(parse_decl_cxt_t *cxt)
{
  ...
  if ((valid = (token != NULL)))
  {
    ...
    {
    case T_LPAREN:
    {                                                          // If the next symbol constitutes a base type then this is func call
    ...
    case T_RPAREN:
      if (parse_exp_isallowed(cxt, token, PARSE_EXP_ALLOWALL))
    ...
    default:
      if (!(token->decl_prop & DECL_QUAL_MASK))
        valid = 0; // Only allow DECL_QUAL and identifier
    }
  }
...
/**
 * @brief Parses the type specifier part of a base type declaration
...
void parse_typespec(parse_decl_cxt_t *cxt, token_t *basetype)
{
  ...
  switch (type)
  { // Basetype declaration cannot be the end of file
  case T_UNSIGNED:
    usign = 1; // Fall through
  /* fall through */
  case T_SIGNED:
    token_destroy(token_get_next(cxt->token_cxt)); // Fall through again
  /* fall through */
  case T_CHAR:
  case T_SHORT:
  case T_INT:
  case T_LONG:
  {                                                  // Note: Do not get_next_token() on these types
    token_t *token = token_get_next(cxt->token_cxt); // unsigned and signed have been processed before this line
    switch (token->type)
    {
    case T_CHAR:
      BASETYPE_SET(basetype, usign ? BASETYPE_UCHAR : BASETYPE_CHAR);
      token_destroy(token);
      return;
    case T_INT:
      BASETYPE_SET(basetype, usign ? BASETYPE_UINT : BASETYPE_INT);
      token_destroy(token);
      return;
    case T_SHORT: // short int has the same effect as short, so we just try to consume an extra int
      BASETYPE_SET(basetype, usign ? BASETYPE_USHORT : BASETYPE_SHORT);
      token_destroy(token);
      token_consume_type(cxt->token_cxt, T_INT);
      return;
    case T_LONG:
    { // long long; long long int; long int; long
      token_destroy(token);
      token_t *token = token_get_next(cxt->token_cxt);
      switch (token->type)
      {
      case T_LONG: // Same as short [int]
        BASETYPE_SET(basetype, usign ? BASETYPE_ULLONG : BASETYPE_LLONG);
        token_destroy(token);
        token_consume_type(cxt->token_cxt, T_INT);
        return;
      case T_DOUBLE:
        ...
        BASETYPE_SET(basetype, BASETYPE_LDOUBLE);
        token_destroy(token);
        return;
      case T_INT:
  ...
  case T_FLOAT:
    BASETYPE_SET(basetype, BASETYPE_FLOAT);
    token_destroy(token_get_next(cxt->token_cxt));
    return;
  case T_DOUBLE:
    BASETYPE_SET(basetype, BASETYPE_DOUBLE);
    token_destroy(token_get_next(cxt->token_cxt));
    return;
  case T_UDEF:
    BASETYPE_SET(ast_append_child(basetype, token_get_next(cxt->token_cxt)), BASETYPE_UDEF);
    return;
  case T_STRUCT:
    BASETYPE_SET(ast_append_child(basetype, parse_comp(cxt)), BASETYPE_STRUCT);
    return;
  case T_UNION:
    BASETYPE_SET(ast_append_child(basetype, parse_comp(cxt)), BASETYPE_UNION);
    return;
  case T_ENUM:
    BASETYPE_SET(ast_append_child(basetype, parse_comp(cxt)), BASETYPE_ENUM);
    return;
  case T_VOID:
    BASETYPE_SET(basetype, BASETYPE_VOID);
    token_destroy(token_get_next(cxt->token_cxt));
    return;
  default:
    assert(0);
...
/**
 * @brief Base type = one of udef/builtin/enum/struct/union; In this stage only allows
...
token_t *parse_decl_basetype(parse_decl_cxt_t *cxt)
{
  ...
  while (token != NULL && (token->decl_prop & DECL_MASK))
  {
    ...
    {
      parse_typespec(cxt, basetype);
    }
    token = token_lookahead(cxt->token_cxt, 1);
  } // Must have some type, cannot be just qualifiers and modifiers
  if (BASETYPE_GET(basetype->decl_prop) == BASETYPE_NONE)
    error_row_col_exit(cxt->token_cxt->s, "Declaration lacks a type specifier\n");
  return basetype;
}
...
token_t *parse_decl(parse_decl_cxt_t *cxt, int hasbasetype)
{
  ...
  while (1)
  {
    ...
    if (token->decl_prop & DECL_QUAL_MASK)
    { // Special case for type qualifiers
    ...
    else
    {
      ...
      case EXP_DEREF: // To avoid int **a*; being legal, because identifiers are not pushed to AST stack
        if (decl_name)
          error_row_col_exit(token->offset, "Pointers can only occur before declared name\n")
              parse_exp_shift(cxt, OP_STACK, token);
        break;
      case T_IDENT: // Trick: Do not push it onto the stack
        if (decl_name)
          error_row_col_exit(token->offset, "Type declaration can have at most one identifier\n");
        decl_name = token;
        break;
        ...
        if(ast_top != NULL && ast_top->type == T_) token_destroy(stack_pop(cxt->stacks[AST_STACK]));
      ...
      case EXP_FUNC_CALL:
      {
        ...
        else
        {
          while (1)
          {
            ...
            { // Special: check "..." after ","
              if (token_lookahead_notnull(cxt->token_cxt, 1)->type == T_ELLIPSIS)
              { // after '...' there can only be ')'
            ...
            else if (token_consume_type(cxt->token_cxt, T_RPAREN))
            {
              break;
            }
            else
              error_row_col_exit(token->offset, "Function declaration expects \')\' or \',\' or \"...\"\n");
          }
        }
      ...
      case EXP_RPAREN:
      {
        ...
        while (op_top != NULL && op_top->type != EXP_LPAREN)
          op_top = parse_exp_reduce(cxt, -1, 0);
        if (op_top == NULL)
          error_row_col_exit(token->offset, "Did not find matching \'(\' in declaration\n");
        token_destroy(stack_pop(cxt->stacks[OP_STACK]));
        token_destroy(token);
        break;
      } // Note that unrelated tokens are filtered
      default:
        printf("%s %s\n", token_typestr(token->type), token->offset);
        assert(0);
      } // switch(token->type)
